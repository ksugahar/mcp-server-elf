"""
ELF MCP tool handlers and compatibility self-test
...
_TOOL_CATALOG = [
    ...
    ("elf_contract_gate(gate, summary_json)", "Canonical typed metadata-gate response"),
    ("elf_usage(topic)", "Curated documentation across 32 topics: "
                          ...
                          "licensing (dongle)"),
    ("elf_help_index / search / get", "Compatibility access to original public summaries"),
    ("elf_examples_index / search / get / playbook", "Compatibility access to original public patterns"),
    ...
    ("elf_wiki_index / search / get", "Public link summaries; vendor page text is not bundled"),
    ("elf_python_index / search / get; elf_python_team28",
                                                  "Public facade-contract summaries plus Python-interface seed manifest"),
]
...
def elf_overview() -> dict:
    ...
    return {
        ...
        "n_prompts": 1,
        "n_resources": len(mcp._resource_manager._resources),
        "n_resource_templates": len(mcp._resource_manager._templates),
        "tool_families": [
        ...
        "recommended_calls": [
            ...
            },
            {
                ...
                "call": "elf_search(query='electrostatic', source='help', top_k=5)",
            },
            {
                "goal": "Browse a large catalog without flooding context",
                "call": "elf_catalog_page(source='samples', offset=0, limit=50)",
            },
            {
...
def elf_agentic_profile() -> dict:
    ...
    return {
        ...
        "source_lane": {
            "public_mcp_server": "ELF/MAGIC public engineering-knowledge MCP package",
...
def _catalog_rows(source: str) -> list[dict]:
    if source == "samples":
        return list_sample_decks()
    if source == "help":
        return list_help_files()
    if source == "examples":
    ...
    raise ValueError("source must be one of samples, help, examples, wiki, or python")


@mcp.tool()
def elf_catalog_page(
    source: Literal["samples", "help", "examples", "wiki", "python"] = "samples",
    offset: PageOffset = 0,
    limit: PageSize = 100,
) -> PageResponse:
    """Return a bounded, typed catalog page from one public source."""
    rows, metadata = page(_catalog_rows(source), offset=offset, limit=limit)
    return PageResponse(source=source, items=rows, **metadata)
...
def elf_search(
    ...
    """Search one public source and return semantic, typed hits."""
    if not query.strip():
        raise ValueError("query must not be empty")
    if source == "samples":
        rows = search_sample_decks(query, top_k=top_k)
        identifier_key, summary_key = "path", "snippet"
    elif source == "help":
        rows = search_help(query, top_k=top_k)
        identifier_key, summary_key = "path", "snippet"
    elif source == "examples":
        rows = search_examples(query, top_k=top_k)
        identifier_key, summary_key = "path", "snippet"
    elif source == "wiki":
        rows = search_wiki(query, top_k=top_k)
        identifier_key, summary_key = "name", "snippet"
    elif source == "python":
        rows = search_python(query, top_k=top_k)
        identifier_key, summary_key = "path", "snippet"
    else:  # Literal is also enforced by the protocol schema.
        raise ValueError("unsupported source")
    hits = []
    for row in rows:
        identifier = str(row.get(identifier_key, ""))
        hits.append(
            SearchHit(
                identifier=identifier,
                title=str(row.get("title") or identifier),
                summary=str(row.get(summary_key, "")),
                score=max(0, int(row.get("score", 0))),
                metadata={k: v for k, v in row.items() if k not in {identifier_key, "title", summary_key, "score"}},
            )
        )
    return SearchResponse(query=query, source=source, hits=hits, total=len(hits))
...
def elf_read(
    ...
    """Read one public artifact or original summary with typed metadata."""
    if source == "samples":
        row = get_sample_deck(identifier, max_chars=max_chars)
        id_key = "path"
    elif source == "help":
        row = get_help_file(identifier, max_chars=max_chars)
        id_key = "path"
    elif source == "examples":
        row = get_example(identifier, max_chars=max_chars)
        id_key = "path"
    elif source == "wiki":
        row = get_wiki_page(identifier, max_chars=max_chars)
        id_key = "name"
    elif source == "python":
        row = get_python_file(identifier, max_chars=max_chars)
        id_key = "path"
    else:
        raise ValueError("unsupported source")
    if "error" in row:
    ...
    return ReadResponse(
        identifier=str(row.get(id_key, identifier)),
        title=str(row.get("title") or row.get(id_key, identifier)),
        text=str(row.get("text", "")),
        truncated=bool(row.get("truncated", False)),
        metadata={k: v for k, v in row.items() if k not in {id_key, "title", "text", "truncated"}},
    )


_STRUCTURED_GATES = {
    "phase_flux": phase_flux_run_contract_gate,
    "ipm_two_run_ldlq": ipm_two_run_ldlq_contract_gate,
    "mesh_solver_pipeline": mesh_solver_pipeline_gate,
    "magnet_model": magnet_model_producer_contract_gate,
    "magnetization_group": magnetization_group_handoff_contract_gate,
    "demagnetization_run": demagnetization_run_contract_gate,
    "transient_induced_current": transient_induced_current_contract_gate,
    "source_off_relaxation": source_off_relaxation_contract_gate,
    "flux_linkage_inductance": flux_linkage_inductance_contract_gate,
    "leakage_inductance": leakage_inductance_contract_gate,
    "emfm_star_power": emfm_star_power_contract_gate,
    "complex_field": complex_field_run_contract_gate,
    "force_pair": force_pair_run_contract_gate,
    "material_force_contrast": material_force_contrast_contract_gate,
    "force_method_profile": force_method_profile_contract_gate,
    "momc_force_triplet": momc_force_triplet_contract_gate,
    "rotating_conductor": rotating_conductor_periodic_contract_gate,
    "two_winding_frequency": two_winding_frequency_contract_gate,
    "conductive_shield_frequency": conductive_shield_frequency_contract_gate,
    "project_feature_inventory": project_feature_inventory_contract_gate,
    "nonlinear_magnetic_conductor": nonlinear_magnetic_conductor_validation_gate,
}


@mcp.tool()
def elf_contract_gate(gate: str, summary_json: str) -> GateResponse:
    """Evaluate a named metadata-only contract and return a typed response."""
    evaluator = _STRUCTURED_GATES.get(gate)
    if evaluator is None:
        raise ValueError(f"unknown gate {gate!r}; available: {', '.join(sorted(_STRUCTURED_GATES))}")
    result = evaluator(summary_json)
    raw_status = str(result.get("status", "")).lower()
    status = "PASS" if raw_status in {"ok", "pass", "passed"} else "FAIL"
    warnings = [str(item) for item in result.get("warnings", [])]
    errors = [str(item) for item in result.get("errors", result.get("issues", []))]
    if status == "FAIL" and not errors:
        errors = [f"gate returned status={raw_status or 'unknown'}"]
    return GateResponse(
        gate=gate,
        status=status,
...
def elf_help_index(prefix: str = "") -> str:
    """
    List original public engineering summaries bundled with this server.
...
@mcp.tool()
def elf_help_search(query: str, top_k: TopK = 10, prefix: str = "") -> str:
    """
    Search original public engineering summaries (case-insensitive).
...
def elf_examples_index(solver: str = "", category: str = "", ext: str = "") -> str:
    """
    List original public input-pattern summaries bundled with this server.
...
@mcp.tool()
def elf_examples_search(query: str, top_k: TopK = 10, solver: str = "", ext: str = "") -> str:
    """
    Search original public input-pattern summaries (case-insensitive).
...
@mcp.tool()
def elf_examples_get(path: str, max_chars: MaxChars = 30000) -> str:
    """
    Get full text of a specific ELF600 example input file.

    Args:
        path: Public pattern identifier returned by ``elf_examples_index``.
...
def elf_sample_decks_index(
    ...
    """
    all_decks = list_sample_decks(family=family or None, case=case or None, ext=ext or None)
    decks, pagination = page(all_decks, offset=offset, limit=limit)
    if not all_decks:
    ...
    header = (
        f"# {pagination['returned']} of {pagination['total']} sample decks "
        f"(offset={pagination['offset']}, limit={pagination['limit']}, has_more={pagination['has_more']})"
...
@mcp.tool()
def elf_sample_decks_search(query: str, top_k: TopK = 10, ext: str = "") -> str:
    """
...
@mcp.tool()
def elf_sample_decks_route(goal: str, limit: PageSize = 5) -> str:
    """
...
@mcp.tool()
def elf_sample_decks_get(path: str, max_chars: MaxChars = 60000) -> str:
    """
...
@mcp.tool()
def elf_sample_decks_playbook(limit: PageSize = 100, family: str = "", query: str = "") -> str:
    """
...
@mcp.tool()
def elf_sample_decks_representatives(area: str = "", limit: PageSize = 36) -> str:
    """
...
@mcp.tool()
def elf_sample_decks_duplicates(family: str = "", max_groups: PageSize = 12) -> str:
    """
...
def elf_python_motor_demag_margin_plan(
    ...
    br_20c_t: PositiveFloat = 1.2,
    br_temp_coeff_pct_per_k: float = -0.11,
    hcj_ka_m: PositiveFloat = 900.0,
    hcj_temp_coeff_pct_per_k: float = -0.45,
    id_min_a_peak: float = -40.0,
    magnet_len_m: PositiveFloat = 0.004,
    gap_csv_m: str = "0.0005,0.001,0.002,0.004,0.008",
    iron_path_m: PositiveFloat = 0.08,
    iron_mu_r: PositiveFloat = 1000.0,
    mu_rec: PositiveFloat = 1.05,
) -> str:
    """
...
def elf_python_ngsolve_validation_plan(
    ...
    motor_type: str = "spm",
    rotor_topology: str = "outer_rotor",
    total_loss_w: PositiveFloat = 25.0,
    base_speed_rpm: PositiveFloat = 3500.0,
    max_speed_rpm: PositiveFloat = 12000.0,
    outer_diameter_mm: PositiveFloat = 80.0,
    stack_length_mm: PositiveFloat = 20.0,
    cooling_h_w_m2k: PositiveFloat = 35.0,
    thermal_conductivity_w_mk: PositiveFloat = 12.0,
    torque_ripple_percent: NonNegativeFloat = 5.0,
    cogging_torque_nm: NonNegativeFloat = 0.02,
    force_order: PositiveFloat = 12.0,
    yield_strength_mpa: PositiveFloat = 450.0,
) -> str:
    """
...
@mcp.tool()
def elf_recipe_search(query: str, top_k: TopK = 5, tag: str = "", solver: str = "") -> str:
    """
...
@mcp.tool()
def elf_help_get(path: str, max_chars: MaxChars = 30000) -> str:
    """
    Get full extracted text of a specific ELF600 help file.

    Args:
        path: Public summary identifier returned by ``elf_help_index`` or
              ``elf_help_search``.
        max_chars: Truncate output if longer (default 30000).
...
# ============================================================
# Public link-summary compatibility tools
# ============================================================

@mcp.tool()
def elf_wiki_index() -> str:
    """
    List original public summaries and vendor links. Vendor page text is not bundled.
    ...
    return f"# {len(pages)} public link summaries\n" + "\n".join(lines)


@mcp.tool()
def elf_wiki_search(query: str, top_k: TopK = 10) -> str:
    """
    Search original public link summaries.
...
@mcp.tool()
def elf_wiki_get(name: str, max_chars: MaxChars = 30000) -> str:
    """
    Get one original public link summary.
...
# ============================================================
# Public Python-facade compatibility tools
# ============================================================

@mcp.tool()
def elf_python_index(ext: str = "") -> str:
    """
    List public Python-facade contract summaries. Vendor wrapper source,
    configuration files, error tables, helper scripts, and binaries are not bundled.
    ...
    header = f"# {len(files)} facade-contract summaries" + (f" (ext={ext})" if ext else " total")
    return header + "\n" + "\n".join(lines)


@mcp.tool()
def elf_python_search(query: str, top_k: TopK = 10, ext: str = "") -> str:
    """
    Search public Python-facade contract summaries.
...
@mcp.tool()
def elf_python_get(path: str, max_chars: MaxChars = 30000) -> str:
    """
    Get one public facade-contract summary.

    Args:
        path: Public facade-contract identifier returned by ``elf_python_index``.
        max_chars: Truncate output if longer (default 30000).
...
@mcp.prompt(title="New ELF Analysis")
def new_elf_analysis(
    geometry: str,
    solver: Literal["MAGIC", "ELFIN", "BEAM"] = "MAGIC",
) -> str:
    ...
    if solver == "MAGIC":
        ...
        return (
            f"Set up an ELFIN electrostatic analysis for: {geometry}\n\n"
            "Workflow:\n"
            "1. Create .mei mesh script (MODEL ELFIN T/K/R)\n"
            "2. Define conductors (ESC), dielectric regions, and potentials\n"
    ...
    raise ValueError("solver must be one of MAGIC, ELFIN, or BEAM")
...
apply_tool_contract(mcp, server_name="elf-mcp-server", version=__version__)
...
def main():
    if "--selftest" in sys.argv:
        ...
        assert n_files == 7, f"Expected 7 original summaries, got {n_files}"
        assert "guides/magic" in idx
        print(f"  {n_files} original summaries indexed")
        idx_guides = elf_help_index("guides/")
        assert "guides/elfin" in idx_guides, "guides/ filter missed ELFIN summary"
        print("  guides/ filter OK")

        # 3. Help search
        print("[3/16] elf_help_search:")
        for q in ["magnetic", "electrostatic", "trajectory", "materials"]:
        ...
        for p in ["guides/magic", "guides/elfin", "guides/beam"]:
            r = elf_help_get(p)
        ...
        assert n_ex == 4, f"Expected 4 original patterns, got {n_ex}"
        magic_idx = elf_examples_index(solver="MAGIC")
        assert "public/magic/" in magic_idx, "MAGIC filter missed public patterns"
        mai_idx = elf_examples_index(ext="mai")
        n_mai = len(list_examples(ext="mai"))
        assert n_mai == 4, f"Expected 4 .mai patterns, got {n_mai}"
        print(f"  {n_ex} examples ({n_mai} .mai), filters OK")

        # 6. Examples search
        print("[6/16] elf_examples_search:")
        for q in ["MOMC", "ELFIN", "BEAM", "PRE"]:
        ...
        for p in ["public/magic/ac_conductor.mai"]:
        ...
        assert pb.count("\n## ") == 4, "Expected 4 original playbook cards"
        magic_pb = elf_examples_playbook(limit=120, solver="MAGIC")
        assert "2 cards" in magic_pb, "Expected 2 original MAGIC cards"
        electrostatic_pb = elf_examples_playbook(limit=20, feature="electrostatic")
        assert "ELFIN" in electrostatic_pb, "Electrostatic filter missed ELFIN pattern"
        print("  4-card original playbook + filters OK")

        # 9. Public sample decks
        print("[9/16] elf_sample_decks tools:")
        sd_page = elf_sample_decks_index(limit=100)
        assert sd_page.startswith("# 100 of ")
        sd = "\n".join(deck["path"] for deck in list_sample_decks())
        ...
        assert len(list_sample_decks(ext="mai")) == 1600, "Expected 1600 public .mai decks"
        ...
        assert "analytic_flux_invariants_passed" in sd_numeric_validation
        ...
        sample_corpus_text = (
            sd + sd_page + sd_search + sd_spm_search + sd_srm_search
        ...
        assert n_w == 2, f"Expected 2 original link summaries, got {n_w}"
        assert "public-boundary" in wi
        print(f"  {n_w} public link summaries")

        print("[12/16] elf_wiki_search:")
        for q in ["public", "solver", "product"]:
        ...
        for p in ["public-boundary", "workflow-selection"]:
            r = elf_wiki_get(p)
        ...
        assert n_p == 3, f"Expected 3 facade summaries, got {n_p}"
        schema_only = elf_python_index(ext="schema")
        assert "facade/MotorSpec" in schema_only and "facade/RunResult" in schema_only
        print(f"  {n_p} facade summaries (schema filter OK)")

        print("[15/16] elf_python_search:")
        for q in ["MotorSpec", "RunRequest", "RunResult"]:
            r = elf_python_search(q, top_k=3)
            assert "No matches" not in r, f"Python query '{q}' had no matches"
        print(f"  3 queries OK")

        print("[16/16] elf_python_get:")
        for p in ["facade/MotorSpec", "facade/RunRequest", "facade/RunResult"]:
            r = elf_python_get(p)
